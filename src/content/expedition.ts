 ...
 *  - 战利品范围：卷星币(常驻) + 星屑·飞船升级材料(常驻) + 星际图鉴卡碎片(一般概率) + 装扮(稀有)
 */

/** 每 30 分钟完成 1 个远征周期，产出 1 个补给包 */
export const LOOT_INTERVAL_MS = 30 * 60 * 1000;
/** 累积（挂机）上限 20 个周期 ≈ 10 小时，满后停止累积 */
export const LOOT_MAX_PACKS = 20;

/** 每包固定产出：卷星币 ↑ 星屑(飞船升级材料) */
export const LOOT_PACK_BEANS = 4;
export const LOOT_PACK_STARDUST = 2;
/** 每包概率掉落：图鉴卡碎片（一般概率）· 装扮（稀有） */
...
export function drawLoot(packs: number): LootDrop {
  const drop: LootDrop = { beans: 0, stardust: 0, cardShard: 0, outfits: [] };
  const outfits = CATALOG.filter((i) => i.kind === 'outfit');
  for (let i = 0; i < packs; i++) {
    drop.beans += LOOT_PACK_BEANS;
    drop.stardust += LOOT_PACK_STARDUST;
    if (Math.random() < LOOT_CARD_CHANCE) drop.cardShard += 1;
